...
// Layout: Breadcrumb → Tool Pills (centered) → 3-col workspace → Full-width gallery
// ─────────────────────────────────────────────────────────────────────────────
...
const USER_CREDITS = 500; // TODO: replace with real balance from Supabase
const POLL_INTERVAL = 4000;
const MAX_POLLS = 60;

// ── Credit estimate (mirrors VideoPromptPanel) ────────────────────────────────
...
const TOOL_MODELS = VIDEO_MODEL_REGISTRY;
          ...
          return (
            <button
              ...
              style={{
                ...
                background: active ? "rgba(14,165,160,0.16)" : "rgba(255,255,255,0.03)",
                color: active ? "#22D3EE" : "#64748B",
                fontSize: 13, fontWeight: active ? 700 : 500,
                cursor: "pointer", transition: "all 0.15s",
                display: "flex", alignItems: "center", gap: 7,
                boxShadow: active ? "0 0 16px rgba(14,165,160,0.2)" : "none",
              }}
              onMouseEnter={e => { if (!active) { (e.currentTarget as HTMLElement).style.color = "#94A3B8"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(255,255,255,0.15)"; } }}
              onMouseLeave={e => { if (!active) { (e.currentTarget as HTMLElement).style.color = "#64748B"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(255,255,255,0.08)"; } }}
            >
              {m.displayName}
              {m.badge && (
                <span style={{
...
function ComingSoonScreen({ model }: { model: VideoModel }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flex: 1, gap: 16, minHeight: 320, textAlign: "center" }}>
      <div style={{
        width: 64, height: 64, borderRadius: 18,
        border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.03)",
        display: "flex", alignItems: "center", justifyContent: "center",
      }}>
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(100,116,139,0.5)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
        </svg>
      </div>
      <div>
        <div style={{ fontSize: 18, fontWeight: 800, color: "#CBD5E1", marginBottom: 8 }}>{model.displayName}</div>
        <div style={{ fontSize: 13, color: "#475569", maxWidth: 300, lineHeight: 1.6 }}>{model.description}</div>
      </div>
      {model.badge && (
        <span style={{
          fontSize: 11, fontWeight: 800, padding: "4px 12px", borderRadius: 20, letterSpacing: "0.06em",
...
function Breadcrumb({ modelName }: { modelName: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 16 }}>
      <a
        href="/studio"
        style={{ fontSize: 12, color: "#475569", textDecoration: "none", transition: "color 0.15s" }}
        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = "#94A3B8"; }}
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = "#475569"; }}
      >
        Studio
      </a>
      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#334155" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <span style={{ fontSize: 12, color: "#64748B", fontWeight: 600 }}>{modelName}</span>
    </div>
  );
}
...
export default function VideoStudioShell() {
  ...
  const [selectedModelId, setSelectedModelId] = useState(
    searchParams.get("model") ?? defaultModelId,
  );
  const model: VideoModel | null = VIDEO_MODEL_REGISTRY.find(m => m.id === selectedModelId) ?? null;
  ...
  // Reset frameMode when switching models if new model doesn't support current mode
  useEffect(() => {
    if (!model) return;
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: "radial-gradient(ellipse at 25% 15%, rgba(14,165,160,0.12) 0%, transparent 55%), radial-gradient(ellipse at 75% 85%, rgba(99,102,241,0.12) 0%, transparent 55%), #020617",
      color: "#E2E8F0",
      fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif",
      paddingTop: 80, // account for sticky navbar
    }}>
      <div style={{ maxWidth: 1440, margin: "0 auto", padding: "24px 28px 0" }}>
        ...
        {/* ── Tool pill bar (centered, max 1100px) ────────────────── */}
        ...
        {(videos.length > 0) && (
          <div style={{
            ...
            <div style={{ fontSize: 11, fontWeight: 700, color: "#475569", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 16 }}>
              Your Videos
            </div>
            <VideoResultsLibrary
              videos={videos}
